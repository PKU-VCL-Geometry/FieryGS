...
from gaussian_renderer import render, network_gui, render_PGSR
import sys, time
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    # backup main code
    import shutil
    for src in ['train_scene_PGSR.py']:
        if os.path.exists(src):
            shutil.copy2(src, dataset.model_path)
    for d in ['arguments', 'gaussian_renderer', 'scene', 'utils']:
        dst = os.path.join(dataset.model_path, d)
        if os.path.isdir(d):
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        render_pkg = render_PGSR(viewpoint_cam, gaussians, pipe, bg, app_model=app_model,
        ...
        # loss += gaussians.compute_Laniso(a=1.1) * 10.0
        ...
        if viewpoint_cam.normal_image is not None and iteration>opt.single_view_weight_from_iter:
            gt_normal = viewpoint_cam.normal_image.cuda()
            render_normal = torch.nn.functional.normalize(render_pkg["rendered_normal"], p=2, dim=0) # [3, H, W]
            ...
            # normal2 = c2w[:3, :3] @ gt_normal.reshape(3, -1)
            # gt_normal_world = normal2.reshape(3, *render_normal.shape[1:])
            # Transform back to camera coordinate system
            # w2c = (viewpoint_cam.world_view_transform.T)
            # normal2 = w2c[:3, :3] @ render_normal.reshape(3, -1)
            # render_normal = normal2.reshape(3, *render_normal.shape[1:])

            loss_prior_normal = l1_loss(gt_normal, render_normal) + (1.0 - torch.sum(render_normal * gt_normal, dim=0)).mean()
            loss += loss_prior_normal * 0.05
        else:
            loss_prior_normal = None
        ...
        if iteration > opt.multi_view_weight_from_iter:
            ...
            if nearest_cam is not None:
                ...
                nearest_render_pkg = render_PGSR(nearest_cam, gaussians, pipe, bg, app_model=app_model,
        ...
        with torch.no_grad():
            ...
            ema_normal_for_log = 0.4 * loss_prior_normal.item() if loss_prior_normal is not None else 0.0 + 0.6 * ema_normal_for_log
            if iteration % 10 == 0:
                loss_dict = {
            ...
            # Log and save
            training_report(tb_writer, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end), testing_iterations, scene, render_PGSR, (pipe, background), app_model)
            ...
            if opt.use_multi_view_trim and iteration % 1000 == 0 and iteration < opt.densify_until_iter:
                ...
                for view in scene.getTrainCameras():
                    render_pkg_tmp = render_PGSR(view, gaussians, pipe, bg, app_model=app_model, return_plane=False, return_depth_normal=False)
