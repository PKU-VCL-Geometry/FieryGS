...
from gaussian_renderer import render_contrastive_feature, render_contrastive_feature_PGSR
import sys
...
from utils.my_utils import print_cuda, FourierScaleGate
import numpy as np
...
def training(dataset, opt, pipe, iteration, saving_iterations, checkpoint_iterations, debug_from):
    ...
    print_cuda("before load gaussian")
    ...
    print_cuda("after load gaussian")
    ...
    # scale_gate = FourierScaleGate(num_frequencies=10, hidden_dim=64, out_dim=32)
    ...
    all_scales = all_scales[~torch.isnan(all_scales)]
    print(all_scales.shape)
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        with torch.no_grad():
            ...
            # print_cuda("after load all masks")
            ...
            # print_cuda("after load all mask scales")
            mask_scales, sort_indices = torch.sort(mask_scales, descending=True) # mask_scales sorted from largest to smallest
            sam_masks = sam_masks[sort_indices, :, :]

            num_sampled_scales = 8
            # Randomly sample 8 scales plus the largest/smallest 10 scales
            ...
            # H W
            per_pixel_mask_size = sam_masks * sam_masks.sum(-1).sum(-1)[:,None,None] # Size of mask each pixel belongs to [N_mask, H, W]

            per_pixel_mean_mask_size = per_pixel_mask_size.sum(dim = 0) / (sam_masks.sum(dim = 0) + 1e-9) # Average mask size across all masks for each pixel

            per_pixel_mean_mask_size = per_pixel_mean_mask_size[sampled_ray] # Extract sampled pixels
            ...
            sam_masks_sampled_ray = sam_masks[:, sampled_ray] #[N_mask, N_sample_pixels]
            ...
            for idx, si in enumerate(sampled_scale_index):
                ...
                if not upper_bound:
                    ...
                    gt_vec[:si+1,:] = sam_masks_sampled_ray[:si+1,:] # Copy values from current si and before (larger masks)
                    for j in range(si, -1, -1): # From smallest to largest
                        gt_vec[j,:] = torch.logical_and(
                            torch.logical_not(gt_vec[j+1:,:].any(dim = 0)), gt_vec[j,:]
                        ) # Ensure each pixel belongs to at most one mask, the smallest mask
                    gt_vec[si+1:,:] = sam_masks_sampled_ray[si+1:,:]
                else:
                    gt_vec = sam_masks_sampled_ray

                gt_corr = torch.einsum('nh,nj->hj', gt_vec, gt_vec) # Value is 1 if two pixels are in the same mask [Sample_pixels, Sample_pixels]
                gt_corr[gt_corr != 0] = 1
            ...
            # N_scale S S
            gt_corrs = torch.stack(gt_corrs, dim = 0) # [N_scale, Sample_pixels, Sample_pixels]

            sampled_scales = q_trans(sampled_scales).squeeze()
            sampled_scales = sampled_scales.squeeze()

        render_pkg_feat = render_contrastive_feature_PGSR(viewpoint_cam, feature_gaussians, pipe, background, return_plane=False, return_depth_normal=False, norm_point_features=True, smooth_type = 'traditional', smooth_weights=torch.softmax(smooth_weights, dim = -1) if smooth_weights is not None else None, smooth_K = opt.smooth_K)
        ...
        feature_with_scale = rendered_features.unsqueeze(0).repeat([sampled_scales.shape[0],1,1,1]) #[N_scales, 32, H, W]
        feature_with_scale = feature_with_scale * gates.unsqueeze(-1).unsqueeze(-1) 

        sampled_feature_with_scale = feature_with_scale[:,:,sampled_ray] #[N_scales, 32, N_sample_pixels]

        scale_conditioned_features_sam = sampled_feature_with_scale.permute([0,2,1]) #[N_scales, N_sample_pixels, 32]

        scale_conditioned_features_sam = torch.nn.functional.normalize(scale_conditioned_features_sam, dim=-1, p=2)
        corr = torch.einsum('nhc,njc->nhj', scale_conditioned_features_sam, scale_conditioned_features_sam) # [N_scales, N_sample_pixels, N_sample_pixels]

        diag_mask = torch.eye(corr.shape[1], dtype=bool, device=corr.device)

        sum_0 = gt_corrs.sum(dim = 0) # [N_sample_pixels, N_sample_pixels] Number of scales where each pixel pair is in the same mask
        consistent_negative = sum_0 == 0 # Pixel pairs that are never in the same mask across all scales
        consistent_positive = sum_0 == len(gt_corrs) # Pixel pairs that are always in the same mask across all scales
        inconsistent = torch.logical_not(torch.logical_or(consistent_negative, consistent_positive)) # The rest
        inconsistent_num = inconsistent.count_nonzero()
        sampled_num = inconsistent_num / 2

        rand_num = torch.rand_like(sum_0)

        # Sample a subset from strong positive/negative pairs for training
