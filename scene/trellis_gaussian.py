...
class TrelllisGaussian:
    ...
    def save_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]], transform_pos=np.array([0,0,0])):
        ...
        if transform is not None:
            ...
            xyz = np.matmul(xyz, transform.T) + transform_pos[None, :]
            rotation = np.quaternion_to_matrix(rotation)
            rotation = np.matmul(transform, rotation)
            rotation = np.matrix_to_quaternion(rotation)
    ...
    def load_ply(self, path, transform=[[1, 0, 0], [0, 0, -1], [0, 1, 0]], transform_pos=np.array([0,0,0])):
        ...
        if transform is not None:
            transform = np.array(transform)
            xyz = np.matmul(xyz, transform) + transform_pos[None, :]
        ...
        global_scale = torch.tensor([1.0,1.5,1.5], device=self.device)
        
        # 1. Position scaling
        xyz = xyz * global_scale[None, :]

        # 2. Size scaling
        # Note: PLY storage is typically in log space (see scale = torch.log(...) in save_ply)
        # So scale * 1.5 in log space corresponds to + log(1.5)
        # scales += np.log(global_scale)
        # convert to _hidden attributes
        self._xyz = xyz #(xyz - self.aabb[None, :3]) / self.aabb[None, 3:]
        ...
        if self.sh_degree > 0:
            ...
            self._features_rest = None
        self._opacity = self.inverse_opacity_activation(opacities) - self.opacity_bias
        self._scaling = self.inverse_scaling_activation(scales)  #self.inverse_scaling_activation(torch.sqrt(torch.square(scales) - self.mininum_kernel_size ** 2)) - self.scale_bias
        self._rotation = rots - self.rots_bias[None, :]
