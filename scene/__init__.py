...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel=None, feature_gaussians: FeatureGaussianModel=None, load_iteration=None, feature_load_iteration=None, shuffle=True, resolution_scales=[1.0], init_from_3dgs_pcd=False, target='scene', mode='train', sample_rate = 1.0, filled=None):
        """b
        ...
        # and not os.path.exists(os.path.join(args.source_path, "transforms.json")):
            ...
            scene_info = sceneLoadTypeCallbacks["Colmap"](args.source_path, args.images, args.eval, need_features = args.need_features, need_masks = args.need_masks, sample_rate = sample_rate, allow_principle_point_shift = args.allow_principle_point_shift, replica = 'replica' in args.model_path, x_rotation=args.x_rotation)
    ...
    def save_feature_attention(self, iteration, target = 'coarse_seg_everything', smooth_weights = None, smooth_type = None, smooth_K = 16):
        assert self.feature_gaussians is not None and (target == 'feature' or target == 'coarse_seg_everything' or target == 'contrastive_feature')
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        self.feature_gaussians.save_ply(os.path.join(point_cloud_path, f"{target}_point_cloud_attention.ply"), smooth_weights, smooth_type, smooth_K)
        
    def save_feature_fourier(self, iteration, target = 'coarse_seg_everything', smooth_weights = None, smooth_type = None, smooth_K = 16):
        assert self.feature_gaussians is not None and (target == 'feature' or target == 'coarse_seg_everything' or target == 'contrastive_feature')
        point_cloud_path = os.path.join(self.model_path, "point_cloud/iteration_{}".format(iteration))
        self.feature_gaussians.save_ply(os.path.join(point_cloud_path, f"{target}_point_cloud_fourier.ply"), smooth_weights, smooth_type, smooth_K)
